...
def jealousy(current):
    for i in range(0, noCouples):
        if (
            current.shore[i] != current.shore[noCouples + i]
...
def h(
    state,
):  # the heuristic function equals the number of people who are not on the goal shore
...
def move(
    cap, state, movement, result, start
...
def GBEST():
    ...
    while True:
        ...
        frontier.sort(
            key=lambda state: h(state)
        )  # sort frontier according to heristic function

        current = frontier.pop(
            0
        )  # examine first item of the frontier (item with the lowest heuristic function)
...
def ASTAR():
    ...
    while True:
        ...
        frontier.sort(
            key=lambda state: state.f()
        )  # sort frontier according to exaluation function

        current = frontier.pop(
            0
        )  # examine first item of the frontier (item with the lowest evaluation function)
